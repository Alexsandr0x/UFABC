...
class Peer():
	...
	def handle_request(self, request):
		requested_file = request['filename']
		address = (address['host'], int(address['port']))

		if requested_file in self.shared_metadata[self.name]['data']:
			self.logger.warning('Arquivo requisitado [{filename}] por endereço [{address}] e com ttl [{ttl}]'.format(
				**request)
			)
		else:
			request['ttl'] -= 1
			if request['ttl'] == 0:
				self.logger.warning(
					'Requisição feita por cliente de endereço [{}] foi expirado por chegar a ttl=0'.format(
						address)
				)
			else:
				random_peer, address = random.choice(list(self.neighbours_peers.items()))
	...
	def metadata_receiver(self):
		...
		while True:
			...
			response = self._deserialize(raw_data)

			if response.get('peer'):
